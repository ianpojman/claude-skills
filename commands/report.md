...
3. **Generate HTML**: Create a themeable, mobile-optimized HTML report using the shared theme framework:
   ...
   - Theme UX: no visible widget; the user presses ⌘K / Ctrl+K to open a command palette — jump to any section (h2/h3), live full-text search the report body, or switch themes (Auto = Aperture light / Carbon dark, persisted in localStorage). Add a footer hint: `Press <kbd>⌘K</kbd> to search, jump to a section, or change theme.`
...
- **⌘K command palette**: empty query shows Sections (every h2/h3, jump + smooth-scroll) and Theme; typing also live full-text searches the report body (snippet results, match highlighted, jump flashes the target). Themes: Auto (default; Aperture-light / Carbon-dark), Soft dark (Slate, Carbon, Dim, Dusk), Deep dark (Graphite, Aurora, Indigo, Teal, Amber, Mono), Light (Aperture, Paper).
